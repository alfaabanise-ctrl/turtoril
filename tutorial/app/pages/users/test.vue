...
const pendingCount = computed(
  () => students.value.filter((s) => s.status === "Pending").length
);
const expiredCount = computed(
  () => students.value.filter((s) => s.status === "Expired").length
);
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
      <div>
        ...
        <p class="text-sm text-gray-400 mt-1">
          All students preparing for JAMB on the platform
        </p>
      </div>
      <UserFormModal type="teacher" buttonText="Create Teacher" />
      <button
    ...
    >
      ...
      <template #cell-mockScore="{ item }">
        <span
          v-if="item.mockScore"
          class="font-semibold"
          :class="scoreColor(item.mockScore)"
        >
          {{ item.mockScore }}/400
        </span>
